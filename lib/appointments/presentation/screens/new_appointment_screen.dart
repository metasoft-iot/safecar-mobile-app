...
import '../../../vehicles/application/vehicles_states.dart';
...
class _NewAppointmentScreenState extends State<NewAppointmentScreen> {
  ...
  void _loadVehicles() {
    ...
    context.read<VehiclesStore>().stream.listen((state) {
  ...
  Future<void> _submitAppointment() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // Wait for appointment creation by listening to the stream
        await for (final state in context.read<AppointmentsStore>().stream) {
          if (state is AppointmentCreated) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('❌ Failed to create appointment: ${state.message}'),
                backgroundColor: Colors.red,
